...
const _memoKey = {};
function concatKey(path, key){
    const concatted =   path.concat(key);
    const join = concatted.join('_');
    if(!_memoKey[join]){
        _memoKey[join] = concatted;
    }
    return _memoKey[join];
}
