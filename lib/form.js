...
module.exports = React.createClass({
  ...
  setValue: function(path, raw, parsed) {
    ...
    if(this.props.update) {
      return this.props.update({
        values: normalise(ou.setIn(this.props.values, path, raw),
                           schema, ctx),
        output: normalise(ou.setIn(this.props.output, path, parsed),
                           schema, ctx)
      });
    }
  ...
  getValue: function(path) {
    if(this.props.update){
        return ou.getIn(this.props.values, path);
    }
    return ou.getIn(this.state.values, path);
  },
  getOutput: function(path) {
    if(this.props.update){
        return ou.getIn(this.props.output, path);
    }
  },
  getErrors: function(path) {
    if(this.props.update){
        return this.props.errors[makeKey(path)];
    }
